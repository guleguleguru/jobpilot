...
const KEYS = {
  ...
  PROFILE_TARGET_DRAFTS: 'profileTargetDrafts',
  SEMANTIC_FIELD_MEMORY: SEMANTIC_FIELD_MEMORY_KEY,
};
...
function normalizeSnapshots(snapshots) {
  ...
  return snapshots
    ...
    .map(snapshot => ({
      ...
      targetDrafts: normalizeProfileTargetDrafts(snapshot.targetDrafts),
    }))
    .sort((left, right) => String(right.createdAt).localeCompare(String(left.createdAt)));
}

function buildProfileSnapshot({ profiles, activeProfileId, reason, siteOverrides, targetDrafts }) {
  return {
    ...
    targetDrafts: normalizeProfileTargetDrafts(targetDrafts),
  };
}
...
function normalizeTargetDraftEntries(entries) {
  if (!entries || typeof entries !== 'object') return {};
  const next = {};
  ...
  for (const [targetKey, entry] of Object.entries(entries)) {
    ...
    const rawPatch = entry?.patch ?? entry;
    const patch = sanitizeProfileOverridePatch(rawPatch);
    if (!normalizedTargetKey || patch === undefined) continue;
    next[normalizedTargetKey] = {
      updatedAt: entry?.updatedAt || new Date().toISOString(),
      patch,
    };
  }
...
function normalizeProfileTargetDrafts(drafts) {
  if (!drafts || typeof drafts !== 'object') return {};
  const next = {};

  for (const [profileId, entries] of Object.entries(drafts)) {
    const normalizedEntries = normalizeTargetDraftEntries(entries);
    if (Object.keys(normalizedEntries).length) next[profileId] = normalizedEntries;
  }
...
async function getProfileTargetDrafts() {
  const result = await chrome.storage.local.get(KEYS.PROFILE_TARGET_DRAFTS);
  return normalizeProfileTargetDrafts(result[KEYS.PROFILE_TARGET_DRAFTS]);
}
...
async function getTargetProfileDraft(profileId, targetKey) {
  const normalizedTargetKey = normalizeTargetKey(targetKey);
  if (!profileId || !normalizedTargetKey) return null;
  const drafts = await getProfileTargetDrafts();
  return drafts[profileId]?.[normalizedTargetKey]?.patch ?? null;
}
...
function targetDraftsEqual(left, right) {
  return JSON.stringify(normalizeProfileTargetDrafts(left) || {}) === JSON.stringify(normalizeProfileTargetDrafts(right) || {});
}

async function saveProfileState({ profiles, activeProfileId, siteOverrides, targetDrafts, snapshotReason } = {}) {
  const current = await chrome.storage.local.get([
    KEYS.PROFILES,
    KEYS.ACTIVE_PROFILE,
    KEYS.PROFILE_SNAPSHOTS,
    KEYS.PROFILE_SITE_OVERRIDES,
    KEYS.PROFILE_TARGET_DRAFTS,
  ]);
  ...
  const currentTargetDrafts = normalizeProfileTargetDrafts(current[KEYS.PROFILE_TARGET_DRAFTS]);
  ...
  const nextTargetDrafts = targetDrafts === undefined
    ? currentTargetDrafts
    : normalizeProfileTargetDrafts(targetDrafts);
  ...
  const targetDraftsChanged = !targetDraftsEqual(currentTargetDrafts, nextTargetDrafts);

  if ((profilesChanged || siteOverridesChanged || targetDraftsChanged) && Object.keys(currentProfiles).length) {
    const nextSnapshots = appendProfileSnapshot(
      ...
      buildProfileSnapshot({
        ...
        targetDrafts: currentTargetDrafts,
      })
    );
  ...
  if (targetDrafts !== undefined) payload[KEYS.PROFILE_TARGET_DRAFTS] = nextTargetDrafts;
...
async function saveProfileTargetDrafts(drafts, options = {}) {
  await saveProfileState({
    targetDrafts: drafts,
    activeProfileId: options.activeProfileId,
    snapshotReason: options.snapshotReason || 'target_profile_draft_update',
  });
}
...
async function getProfile(siteKey = '', targetKey = '') {
  const [profiles, activeId] = await Promise.all([getProfiles(), getActiveProfileId()]);
  if (!profiles) return null;
  ...
  const normalizedSiteKey = normalizeSiteKey(siteKey);
  const normalizedTargetKey = normalizeTargetKey(targetKey);
  let mergedProfile = normalizedSiteKey
    ? mergeProfileWithOverride(baseProfile, await getSiteProfileOverride(activeId, normalizedSiteKey))
    : normalizeProfile(baseProfile);

  if (normalizedTargetKey) {
    mergedProfile = mergeProfileWithTargetDraft(
      mergedProfile,
      await getTargetProfileDraft(activeId, normalizedTargetKey)
    );
  }
...
async function deleteProfile(id) {
  ...
  const targetDrafts = await getProfileTargetDrafts();
  if (!profiles || !profiles[id]) throw new Error(`Profile ${id} does not exist`);
  ...
  if (targetDrafts[id]) delete targetDrafts[id];
...
async function saveTargetProfileDraft(profileId, targetKey, patch, options = {}) {
  const normalizedTargetKey = normalizeTargetKey(targetKey);
  const normalizedPatch = sanitizeProfileOverridePatch(patch);
  if (!profileId) throw new Error('Profile id is required');
  if (!normalizedTargetKey) throw new Error('Target key is required');

  const drafts = await getProfileTargetDrafts();
  if (normalizedPatch === undefined) {
    if (drafts[profileId]?.[normalizedTargetKey]) {
      delete drafts[profileId][normalizedTargetKey];
      if (!Object.keys(drafts[profileId]).length) delete drafts[profileId];
      await saveProfileTargetDrafts(drafts, { snapshotReason: 'target_profile_draft_delete' });
    }
    return;
  }

  drafts[profileId] = drafts[profileId] || {};
  const existingPatch = drafts[profileId][normalizedTargetKey]?.patch;
  drafts[profileId][normalizedTargetKey] = {
    updatedAt: new Date().toISOString(),
    patch: options.merge === false ? normalizedPatch : mergeSiteOverridePatch(existingPatch, normalizedPatch),
  };
  await saveProfileTargetDrafts(drafts, { snapshotReason: 'target_profile_draft_save' });
}
...
async function restoreProfileSnapshot(snapshotId) {
  ...
  await chrome.storage.local.set({
    ...
    [KEYS.PROFILE_TARGET_DRAFTS]: normalizeProfileTargetDrafts(snapshot.targetDrafts),
...
async function getSemanticFieldMemory() {
  const result = await chrome.storage.local.get(KEYS.SEMANTIC_FIELD_MEMORY);
  return normalizeSemanticFieldMemory(result[KEYS.SEMANTIC_FIELD_MEMORY]);
}

async function saveSemanticFieldMemory(entries) {
  await chrome.storage.local.set({
    [KEYS.SEMANTIC_FIELD_MEMORY]: normalizeSemanticFieldMemory(entries),
  });
}

async function learnSemanticFieldMemorySamples(samples = []) {
  if (!Array.isArray(samples) || !samples.length) return [];
  const current = await getSemanticFieldMemory();
  const next = learnSemanticFieldMemory(current, samples);
  await saveSemanticFieldMemory(next);
  return next;
}
