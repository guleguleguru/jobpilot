...
function sanitizeProfile(profile) {
  ...
  if (Array.isArray(profile.jobPreferences?.interviewLocations) && profile.jobPreferences.interviewLocations.length) {
    jobPreferences.interviewLocations = profile.jobPreferences.interviewLocations.slice(0, 3);
  } else if (profile.jobPreferences?.interviewCity) {
    jobPreferences.interviewLocations = [profile.jobPreferences.interviewCity];
  }
  ...
  if (jobPreferences.interviewLocations?.length) {
    clean.jobPreferences.interviewCity = jobPreferences.interviewLocations[0];
  }
  ...
  const arrays = [
    ...
    ['competitions', ['name', 'level', 'award', 'date', 'description']],
    ['developerLanguages', ['name', 'level']],
...
function buildTargetProfilePrompt(jobContext = {}, profile = {}) {
  const safeProfile = JSON.stringify(sanitizeProfile(profile), null, 2);
  const safeContext = JSON.stringify({
    company: String(jobContext.company || '').trim(),
    role: String(jobContext.role || '').trim(),
    notes: String(jobContext.notes || '').trim(),
  }, null, 2);

  return [
    {
      role: 'system',
      content: `你是求职简历定制助手。请根据岗位上下文，把通用资料改写成更贴合岗位的“稀疏 JSON Patch”。
规则：
1. 只返回稀疏 JSON Patch，只包含需要覆盖或补强的字段。
2. 禁止编造不存在的经历、学校、公司、奖项、成绩或证件信息。
3. 可以改写 selfIntro、jobPreferences、projects[].description、experience[].description、skills、developerLanguages[] 等非敏感字段。
4. 不要输出身份证、家庭成员、婚姻、健康、政治面貌、户籍等敏感字段。
5. 如果没有必要改动，返回 {}。
6. 只返回 JSON，不要解释。`,
    },
    {
      ...
      content: `岗位上下文：\n${safeContext}\n\n当前通用资料：\n${safeProfile}\n\n请返回稀疏 JSON Patch，例如：
{
  "selfIntro": "更贴合岗位的自我介绍",
  "jobPreferences": {
    "expectedPositions": ["算法工程师"]
  },
  "projects": [
    {
      "description": "更强调与岗位相关的项目亮点"
    }
  ]
}`,
    },
...
export { buildFieldMappingPrompt, buildFormSelectionPrompt, buildTargetProfilePrompt, sanitizeProfile, validateFieldMappings };
